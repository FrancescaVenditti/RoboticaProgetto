...
int main(int argc, char * argv[])
{
  ...
  auto const node = std::make_shared<rclcpp::Node>(
    "franka_moveit_planning_demo",
    rclcpp::NodeOptions().automatically_declare_parameters_from_overrides(true)
  );

  // Create the ROS logger
  auto const logger = rclcpp::get_logger("franka_moveit_planning_demo");

  // Spin up a SingleThreadedExecutor for MoveItVisualTools to interact with ROS
  rclcpp::executors::SingleThreadedExecutor executor;
  executor.add_node(node);
  
  // Avvia un thread separato per far girare l'executor
  ...
  // Wait for the parameters to be available
  /* if (!node->has_parameter("robot_description") || !node->has_parameter("robot_description_semantic"))
  {
    RCLCPP_FATAL(logger, "Missing required parameters: robot_description or robot_description_semantic");
    return 1;
  }*/

  // Initialize RobotModelLoader
  /*auto robot_model_loader = std::make_shared<robot_model_loader::RobotModelLoader>(node, "robot_description", true);
  auto kinematic_model = robot_model_loader->getModel();*/
 
 /* if (!kinematic_model)
  {
    RCLCPP_FATAL(logger, "Unable to construct robot model. Please make sure all needed information is on the parameter server.");
    return 1;
  }*/
  ...
  auto const execute = [&logger, &move_group_interface, &moveit_visual_tools, 
    &draw_title, &prompt, &draw_trajectory_tool_path]
    (auto const success, auto const plan)
  {
    if (success) 
    {
